...
* *PsuedoRandomizerTargetPurgeGoals* : Order of the room goals during the Purge Event Maze that the mod's *psuedo-randomizer* will target when generating a new seed
    * Default value is 'RLRLRL' (Right, Left, Right, Left, Right, Left)
        * Each character in the input represents a goal direction, valid directions are 'R' (Right), 'L' (Left), 'F' (Foward), and 'A' (Any)
...
* *ReloadSaveWithRandomSeed_Key* : Key used to reload the current save with a randomized seed
    * Default value is 'None'

* *ReloadSaveWithPsuedoRandomSeed_Key* : Key used to reload the current save with a *psuedo-randomized* seed
    * Default value is 'None'

...
namespace KatieSaveHelper.PsuedoRandomizer
{
    ...
    public class ObstacleCondition
    {
        ...
        public bool EvaluateVerbose(PurgeObstacle value)
        {
            ...
            {
                case LogicType.Any:
                    MelonLogger.Msg("Logic: Any, returning True");
                    return true;
                case LogicType.MustBe:
                    MelonLogger.Msg($"Logic: MustBe, returning {valueSet.Contains(value)}");
                    return valueSet.Contains(value);
                case LogicType.MustNotBe:
                    MelonLogger.Msg($"Logic: MustNotBe, returning {!valueSet.Contains(value)}");
                    return !valueSet.Contains(value);
                default:
                    MelonLogger.Msg($"Logic: Default, returning False");
                    return false;
            }
        }
    ...
    internal class KatiePsuedoRandomizer
    {
        ...
        {
            isBusy = true;
            MelonLogger.Msg("Searching for seed...");
            ...
            {
                MelonLogger.Msg("Seed not found.");
                MelonLogger.Msg($"Searched {attemptCount} total seeds");
                MelonLogger.Msg($"Fails : Head - {headFails}, Rooms - {roomFails}, Obstacles - {obstacleFails}");
                return (false, 0);
            }
            else
            {
                MelonLogger.Msg($"Seed found! {currentSeed}");
                MelonLogger.Msg($"Searched {attemptCount} total seeds");
        ...
        static bool EvaluatePurgeObstaclesVerbose(int baseHash, List<ObstacleCondition> conditions)
        {
            ...
            {
                randIndex = random.Next(0, 8);
                MelonLogger.Msg($"Rand is {randIndex}");
                PurgeObstacle popsicle = (PurgeObstacle)randIndex;
                MelonLogger.Msg($"Rand obstacle is {popsicle}");
                if (!condition.EvaluateVerbose(popsicle))
                {
                    MelonLogger.Msg($"Eval Returned False");
                    return false;

                }
                random.Next();
            }
            MelonLogger.Msg($"Eval Returned True");
            return true;
        }
        ...
        static List<PurgeDirection> ParseRoomOrder(string input)
        {
            ...
            if (hasInvalidInput)
            {   
                MelonLogger.Warning($"The loaded mod config entry for the setting '{KatieSaveHelperModConfig.psuedoRandomTargetPurgeRoomGoals.InternalName}' represents an impossible goal direction order for the Purge Event Maze : '{KatieSaveHelperModConfig.psuedoRandomTargetPurgeRoomGoals.Value}'");
                MelonLogger.Warning("Impossible goal directions have been replaced with 'A' (Any)");
            }
